...
__global__ void Particle_Cell_Kernel(float4* P, int ng, float4* C, int nl, float3* F, float epsilon) {
    ...
    if(id < ng * nl){
        ...
        float F_mag = -(P[ip].w * C[ic].w) / powf(r_sq + epsilon*epsilon, 1.5);
        atomicAdd(&F[ip].x, F_mag * dx.x);
        atomicAdd(&F[ip].y, F_mag * dx.y);
        atomicAdd(&F[ip].z, F_mag * dx.z);
    }
    // Particle-Particle
    else if(id < ng * (nl + ng)){
        id -= ng * nl;
        ...
        dx.z = P[ip].z - P[ipp].z;
        r_sq = dx.x * dx.x + dx.y * dx.y + dx.z * dx.z;
        float F_mag = -(P[ip].w * P[ipp].w) / powf(r_sq + epsilon*epsilon, 1.5);
        atomicAdd(&F[ip].x, F_mag * dx.x);
        atomicAdd(&F[ip].y, F_mag * dx.y);
        atomicAdd(&F[ip].z, F_mag * dx.z);
    }
